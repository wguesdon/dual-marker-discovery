...
**The task `.md` files are always committed**, and a workbench deliverable joins them in `outputs/` when it
can actually change the report. Everything else stays local: screenshots (`img/`) and bulk exports
(`results/`) are gitignored on purpose, since the task files already carry the prompts and the text answers.
A two or three line summary of a run that should change the report goes into `docs/claude_tooling_log.md`.

Anything in `outputs/` is a literature synthesis, not pipeline output. It is not regenerated by `make`, and
a claim from it enters the report only once checked against the cited primary source. See
`outputs/README.md`. The workbench kernel code behind those deliverables is reconstructed in `scripts/`,
for provenance: it runs inside Claude Science, not here. See `scripts/README.md`.
